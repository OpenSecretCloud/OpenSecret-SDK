...
async fn test_session_establishment() -> Result<()> {
    let base_url = env::var("OPENSECRET_TEST_URL")
        .unwrap_or_else(|_| "http://localhost:3000".to_string());
    
    let client = OpenSecretClient::new(base_url)?;
    
    // Initially should have no session
    assert!(client.get_session_id()?.is_none(), "Should not have session before handshake");
    ...
    assert!(session_id.to_string().len() > 20, "Session ID should be a reasonable length");
...
async fn test_session_key_derivation() -> Result<()> {
    use opensecret_sdk::crypto::{generate_key_pair, derive_shared_secret};
