use opensecret_sdk::{OpenSecretClient, Result, Error};
use std::env;
...
async fn test_attestation_handshake_production() -> Result<()> {
    ...
    if base_url.contains("localhost") || base_url.contains("127.0.0.1") {
        return Err(Error::Configuration("Production test requires non-localhost URL".to_string()));
    }
...
async fn test_attestation_nonce_verification() -> Result<()> {
    ...
    assert_ne!(first_session, second_session, "Each handshake should create a unique session");
